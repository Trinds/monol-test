<div class="top-bar">
    <form method="get" action="/{{ $dir }}">
        ...
        <div class="search-input">
            <input type="search" name="searchParam" id="searchParam" class="custom-icon" placeholder="Pesquisar..." value={{request('searchParam')}}>
        </div>
        @if(isset($filterName))
            <div>
                ...
                <select class="form-control" id="filter" name="filter" onchange="this.form.submit()">
                    ...
                    @if( isset($courses)  && $dir=='classrooms' )
                        @foreach ($courses as $course)
                            <option value={{ $course->id }} {{ request('filter') == $course->id ? 'selected' : '' }}>{{ $course->abbreviation }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
