<div class="student-card">
    ...
    <form method="POST" action="{{ route('students.update', ['student' => $student]) }}">
        <fieldset>
            ...
            <!-- Classroom View-->
            <label id=studentClassroomView>Turma:<input type="text" value="{{$student->classroom->course->abbreviation . $student->classroom->edition}}" readonly></label>
            <!-- Classroom Choice-->
            <div id="classroomDropdowns" hidden>
                <label> Curso:
                    <select class="selectForm" id="course_id">
                        @foreach($courses as $course)
                        <option value="{{$course->id}}" @if ($course->abbreviation === $student->classroom->course->abbreviation)
                            selected
                            @endif
                ...
                <label>Edição:
                    <select class="selectForm" name="classroom_id" id="classroom_id">
                        ...
                        <option value="{{$classroom->id}}" data-course="{{$classroom->course_id}}" @if ($classroom->edition === $student->classroom->edition)
                            selected
                            @endif
            ...
            <!-- Student Number -->
            <label>Nº Aluno:<input type="text" name="student_number" value="{{$student->student_number}}" id="studentNumberInput" readonly></label>
            <!-- Birth Date -->
